...
public class PersonajeController : MonoBehaviour
{
    ...
    void Update()
    {
        if (laser == true)
        {
            colornave.GetComponent<Renderer>().material.color = Color.red;
        }
        else
        {
            colornave.GetComponent<Renderer>().material.color = Color.white;
        }
        enableShot();
        //move();
    }
   ...
   // }
    ...
    void enableShot()
    {
        ...
        if (Input.GetKey(KeyCode.Return) && Time.time > nextFire && !ScenesController.childMode)
        {
            ...
            if (laser)
            {
                Instantiate(shotlaser, shotSpawn.position, shotSpawn.rotation);
                laser = false;
            }
            else
            {
